<x-app-layout>
    ...
    <div class="py-10"><div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <form method="GET" data-live-search="#company-results" class="mt-5">
            <input type="search" autocomplete="off" name="search" value="{{ request('search') }}" placeholder="Search companies..." class="w-full max-w-sm rounded-xl border-gray-200 p-3 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
        </form>

        <div id="company-results"><div class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div class="overflow-x-auto"><table class="min-w-full divide-y divide-slate-200 text-sm">
        ...
        </table></div></div><div class="mt-5">{{ $companies->links() }}</div></div>
    </div></div>
</x-app-layout>
