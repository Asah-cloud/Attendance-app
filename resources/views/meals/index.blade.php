    ...
    <div class="grid gap-5 lg:grid-cols-2">
        @forelse($meals as $meal)
            <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div><p class="text-xs font-black uppercase tracking-wider {{ $meal->isOpen() ? 'text-emerald-600' : 'text-slate-400' }}">{{ $meal->isOpen() ? 'Open now' : 'Closed' }}</p><h3 class="mt-2 text-xl font-black">{{ $meal->name }}</h3><p class="mt-2 text-xs text-slate-500">{{ $meal->opens_at?->format('M j, g:i A') ?? 'No opening time' }} — {{ $meal->closes_at?->format('M j, g:i A') ?? 'No closing time' }}</p></div>
                <livewire:meal-distribution-stats :event="$event" :meal="$meal" :key="'meal-stats-'.$meal->id" />
