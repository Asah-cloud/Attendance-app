<x-app-layout>
    <x-slot name="header">Staff check-in scanner</x-slot>
        ...
        <div class="mx-auto max-w-3xl px-4 sm:px-6">
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                    <div>
                        <p class="text-xs font-black uppercase tracking-[0.18em] text-amber-600">Badge collection · Staff scanner</p>
                        <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950">{{ $event->title }}</h1>
                        <p class="mt-2 text-sm text-slate-500">Scan each staff member's badge QR code once, when they collect it. This does not affect daily attendee attendance.</p>
                    </div>
                </div>
                ...
                <form id="manual-scan" class="mt-8 border-t border-slate-100 pt-6">
                    <label for="registration-code" class="text-xs font-black uppercase tracking-widest text-slate-500">Or enter the staff code</label>
                    <div class="mt-3 flex flex-col gap-3 sm:flex-row">
                        <input id="registration-code" class="min-w-0 flex-1 rounded-xl border-slate-300" placeholder="Paste staff QR value" required>
                        <button class="rounded-xl bg-amber-600 px-5 py-3 text-sm font-extrabold text-white hover:bg-amber-700">Check in</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            const checkIn = async (value) => {
                ...
                busy = true;
                try {
                    const response = await fetch(@js(route('support-staff.checkin.scan', $event)), {
                        method: 'POST',
            ...
            if (window.loadHtml5Qrcode) {
                ...
                scanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 240, height: 240 } }, checkIn)
                    .catch(() => showResult('We could not open the camera. Please allow camera access, or enter the staff code below.', false));
            }
        });
    </script>
    @endpush
