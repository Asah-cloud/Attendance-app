<x-app-layout>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
            ...
            @if($company->isPayPerEvent())
                <section class="rounded-3xl bg-[#071426] p-8 text-white shadow-xl">
                    <p class="text-xs font-black uppercase tracking-[0.2em] text-blue-300">Billing mode</p>
                    <p class="mt-2 text-sm font-bold text-slate-300">{{ $company->name }}</p>
                    <h3 class="mt-3 text-4xl font-black">Pay per event</h3>
                    <p class="mt-2 max-w-2xl text-slate-300">No monthly fee, no event cap. Each event is billed per confirmed attendee when it finalizes — see the bills below, or open an event's own Billing tab to finalize and pay one.</p>
                </section>

                <section class="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
                    <div class="border-b border-gray-100 p-6"><h3 class="font-black text-gray-900">Attendee bills</h3></div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm">
                            <thead class="bg-gray-50 text-xs uppercase text-gray-500"><tr><th class="p-4">Event</th><th class="p-4">Status</th><th class="p-4">Attendees</th><th class="p-4 text-right">Amount</th></tr></thead>
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @empty
                                    <tr><td colspan="4" class="p-8 text-center text-gray-500">No attendee bills yet — they're created automatically once an event's date arrives.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <section>
                    <h3 class="mb-4 text-lg font-black text-gray-900">Prefer a monthly plan instead?</h3>
                    <div class="grid gap-5 md:grid-cols-3">
                        @foreach($plans as $key => $plan)
                            <article class="group rounded-3xl border border-gray-100 bg-white p-6 shadow-sm hover:-translate-y-1 hover:shadow-xl">
                                ...
                                <a href="{{ route('billing.checkout', $key) }}" class="mt-6 block rounded-xl bg-blue-600 px-4 py-3 text-center text-sm font-bold text-white">Switch to this plan</a>
                            </article>
                        @endforeach
                    </div>
                </section>
