<x-app-layout>
    ...
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-black text-slate-900">{{ isset($message) ? 'Edit and resend message' : 'Compose a custom message' }}</h1>
                <p class="text-sm text-slate-500">{{ $event->title }}@isset($message) · Original message remains in history; sending creates a new record.@endisset</p>
            </div>
        ...
        <form method="POST" action="{{ isset($message) ? route('events.messages.resend', [$event, $message]) : route('events.messages.store', $event) }}" enctype="multipart/form-data" class="space-y-8"
              x-data="{ selected: new Set(@js(array_map('strval', $selectedParticipantIds ?? []))), search: '' }">
            @csrf
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4"><label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Subject</label>
                    <input type="text" name="subject" value="{{ old('subject', $message->subject ?? '') }}" class="w-full rounded-xl border-gray-200 text-sm" placeholder="e.g. Important update about the event">
                </div>
                <div class="mt-4"><label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Email body</label>
                    <textarea name="email_body" rows="6" class="w-full rounded-xl border-gray-200 text-sm" placeholder="Type the email message...">{{ old('email_body', $message->email_body ?? '') }}</textarea>
                </div>
                <div class="mt-4">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4"><label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">SMS message</label>
                    <textarea name="sms_body" rows="3" maxlength="1000" class="w-full rounded-xl border-gray-200 text-sm" placeholder="Type the SMS message...">{{ old('sms_body', $message->sms_body ?? '') }}</textarea>
                </div>
            </section>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/60 p-4">
                        <input type="radio" name="mode" value="smart" @checked(old('mode', $message->mode ?? 'smart') === 'smart') class="mt-1">
                    ...
                    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <input type="radio" name="mode" value="both" @checked(old('mode', $message->mode ?? '') === 'both') class="mt-1">
                    ...
                    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <input type="radio" name="mode" value="email_only" @checked(old('mode', $message->mode ?? '') === 'email_only') class="mt-1">
                    ...
                    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <input type="radio" name="mode" value="sms_only" @checked(old('mode', $message->mode ?? '') === 'sms_only') class="mt-1">
            ...
            <div class="flex justify-end">
                <button type="submit" class="rounded-xl bg-blue-900 px-8 py-3.5 text-sm font-black uppercase tracking-widest text-white shadow-lg hover:bg-blue-800">{{ isset($message) ? 'Resend message' : 'Send message' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
