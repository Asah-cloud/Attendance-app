    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <details class="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" @if($errors->hasAny(['name','email','phone','category'])) open @endif>
            ...
            <form method="POST" action="{{ route('events.registrations.store', $event) }}" class="mt-5 grid gap-4 md:grid-cols-2">@csrf
                ...
                <input name="category" value="{{ old('category', 'Member') }}" required placeholder="Attendee type" class="rounded-xl border-slate-200">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div class="overflow-x-auto"><table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-slate-50 text-left text-xs font-black uppercase tracking-wider text-slate-500"><tr><th class="px-5 py-4">Attendee</th><th class="px-5 py-4">Contact</th><th class="px-5 py-4">Type</th><th class="px-5 py-4">Status</th><th class="px-5 py-4">Registered</th><th class="px-5 py-4">Actions</th></tr></thead>
            <tbody class="divide-y divide-slate-100">@forelse($registrations as $registration)
                <tr><td class="px-5 py-4 font-bold text-slate-900">{{ $registration->participant->name }}</td><td class="px-5 py-4 text-slate-600">{{ $registration->participant->email ?: '—' }}<br>{{ $registration->participant->phone ?: '' }}</td><td class="px-5 py-4">{{ $registration->participant->category }}</td><td class="px-5 py-4"><span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{{ ucfirst($registration->status) }}</span></td><td class="px-5 py-4 text-slate-500">{{ $registration->registered_at?->format('M j, Y g:i A') }}</td>
            ...
            @empty<tr><td colspan="6" class="px-5 py-12 text-center text-slate-500">No registrations found.</td></tr>@endforelse</tbody>
