<x-app-layout>
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <form id="attendee-filter" method="GET" action="{{ route('events.registrations.index', $event) }}" class="mb-5 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end">
            ...
            <div>
                <label for="attendee-status" class="mb-1 block text-xs font-black uppercase tracking-wider text-slate-500">Status</label>
                ...
                <tr id="edit-participant-{{ $registration->id }}" class="hidden bg-slate-50">
                    <td colspan="7" class="px-5 py-5">
                        <form method="POST" action="{{ route('events.registrations.participant.update', [$event, $registration]) }}" class="grid gap-3 md:grid-cols-2 lg:grid-cols-6">
                            ...
                            <input name="room_group" value="{{ old('room_group', $registration->participant->room_group) }}" placeholder="Area / Group" class="rounded-xl border-slate-200 text-sm">
